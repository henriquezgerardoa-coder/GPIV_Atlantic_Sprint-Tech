(function (factory) {
    ...
    L.DivOverlay.include({
         ...
         */
        getEvents: function () {
            return L.extend(divOverlayProto.getEvents.apply(this, arguments), {
        ...
        _updatePosition: function () {
            ...
            if (this._map && this._map._rotate && this._zoomAnimated) {
                ...
                var pos = L.DomUtil.getPosition(this._container).subtract(
                    anchor,
                );
                L.DomUtil.setPosition(
                    this._container,
                    this._map.rotatedPointToMapPanePoint(pos).add(anchor),
                );
            }
        },
    });
    ...
    L.Popup.include({
        ...
        _adjustPan: function () {
            ...
            var map = this._map,
                marginBottom =
                    parseInt(
                        L.DomUtil.getStyle(this._container, "marginBottom"),
                        10,
                    ) || 0,
    ...
    L.GridLayer.include({
        ...
        _getTiledPixelBounds: function (center) {
            if (!this._map._rotate) {
                return gridLayerProto._getTiledPixelBounds.apply(
                    this,
                    arguments,
                );
            }
    ...
    L.Renderer.include({
        ...
        _update: function () {
            ...
            this._bounds = this._map._getPaddedPixelBounds(
    ...
    L.Map.include({
        ...
        containerPointToLayerPoint: function (point) {
            if (!this._rotate) {
                return mapProto.containerPointToLayerPoint.apply(
                    this,
                    arguments,
                );
            }
        ...
        layerPointToContainerPoint: function (point) {
            if (!this._rotate) {
                return mapProto.layerPointToContainerPoint.apply(
                    this,
                    arguments,
                );
            }
        ...
        mapBoundsToContainerBounds: function (bounds) {
            if (!this._rotate && mapProto.mapBoundsToContainerBounds) {
                return mapProto.mapBoundsToContainerBounds.apply(
                    this,
                    arguments,
                );
            }
            ...
            const nw = this.layerPointToContainerPoint(
                    this.project(bounds.getNorthWest())._subtract(origin),
                ),
                ne = this.layerPointToContainerPoint(
                    this.project(bounds.getNorthEast())._subtract(origin),
                ),
                sw = this.layerPointToContainerPoint(
                    this.project(bounds.getSouthWest())._subtract(origin),
                ),
                se = this.layerPointToContainerPoint(
                    this.project(bounds.getSouthEast())._subtract(origin),
                );

            return L.bounds([
                L.point(
                    Math.min(nw.x, ne.x, se.x, sw.x),
                    Math.min(nw.y, ne.y, se.y, sw.y),
                ), // [ minX, minY ]
                L.point(
                    Math.max(nw.x, ne.x, se.x, sw.x),
                    Math.max(nw.y, ne.y, se.y, sw.y),
                ), // [ maxX, maxY ]
            ]);
        },
        ...
        setBearing: function (theta) {
            ...
            var bearing =
                    L.Util.wrapNum(theta, [0, 360]) * L.DomUtil.DEG_TO_RAD,
                center = this._getPixelCenter(),
                oldPos = this._getRotatePanePos().rotateFrom(
                    -this._bearing,
                    center,
                ),
        ...
        panInside(latlng, options) {
            ...
            const paddingTL = L.point(
                ...
                pixelBounds = L.bounds([
                    L.point(rect),
                    L.point(rect).add(this.getSize()),
                ]),
                pixelCenter = pixelBounds.getCenter(),
                //
                paddedBounds = L.bounds([
            ...
            if (!paddedBounds.contains(pixelPoint)) {
                ...
                const centerOffset = pixelPoint.subtract(
        ...
        _getNewPixelBounds: function (center, zoom) {
            ...
            var mapZoom = this._animatingZoom
                    ? Math.max(this._animateToZoom, this.getZoom())
                    : this.getZoom(),
        ...
        addHooks: function () {
            ...
            if (!this._observer) {
                this._observer = new MutationObserver(
                    L.Util.bind(this._map.invalidateSize, this._map),
                );
            }
    ...
    L.Map.TouchGestures = L.Handler.extend({
        ...
        _onTouchMove: function (e) {
            ...
            if (this._rotating) {
                ...
                var bearingDelta =
                    (theta - this._startTheta) * L.DomUtil.RAD_TO_DEG;
                if (vector.y < 0) {
                    bearingDelta += 180;
                }
                if (bearingDelta) {
        ...
        _onTouchEnd: function () {
            ...
            L.DomEvent.off(document, "touchmove", this._onTouchMove, this).off(
                document,
                "touchend touchcancel",
                this._onTouchEnd,
                this,
