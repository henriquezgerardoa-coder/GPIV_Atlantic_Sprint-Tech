(async () => {
    ...
    if (!token) {
        ...
        ayuda.textContent =
            "Solicita un nuevo correo de verificacion desde la pantalla de registro.";
        return;
    }

    try {
        const respuesta = await fetch(
            "/api/public/verificacion?token=" + encodeURIComponent(token),
        );
        ...
        if (respuesta.ok) {
            ...
            ayuda.textContent =
                "Tu cuenta ya esta activa. Continúa al ingreso para acceder al sistema.";
            return;
        }
        const mensaje =
            cuerpo.mensaje ||
        ...
        actualizarVista(
            "danger",
            "No se pudo contactar al servidor para verificar el token.",
            {
        ...
        ayuda.textContent =
            "Verifica tu conexion y vuelve a intentarlo en unos minutos.";
    }
})();
