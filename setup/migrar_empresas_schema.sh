...
SCRIPT_SQL="${RAIZ_PROYECTO}/setup/sql/004_alinear_tabla_empresas.sql"
...
if [ -n "${DB_CONTAINER:-}" ]; then
  echo "Aplicando migracion de esquema empresas en contenedor '${DB_CONTAINER}' (db=${DB_NAME}, user=${DB_USER})..."
  docker exec -i "${DB_CONTAINER}" psql -v ON_ERROR_STOP=1 -U "${DB_USER}" -d "${DB_NAME}" < "${SCRIPT_SQL}"
  echo "Migracion aplicada."
  exit 0
fi

echo "Aplicando migracion de esquema empresas con cliente local psql (db=${DB_NAME}, user=${DB_USER})..."
psql \
  -v ON_ERROR_STOP=1 \
  -h "${DB_HOST:-127.0.0.1}" \
  -p "${DB_PORT:-5432}" \
