...
SCRIPT_SQL="${RAIZ_PROYECTO}/setup/sql/005_habilitar_lotes_sin_empresa.sql"
...
if [ -n "${DB_CONTAINER:-}" ]; then
  echo "Aplicando migracion de lotes sin empresa en contenedor '${DB_CONTAINER}' (db=${DB_NAME}, user=${DB_USER})..."
...
if command -v psql >/dev/null 2>&1 && psql --version >/dev/null 2>&1; then
  echo "Aplicando migracion de lotes sin empresa con cliente local psql (db=${DB_NAME}, user=${DB_USER})..."
