...
SCRIPT_SQL="${RAIZ_PROYECTO}/setup/sql/008_habilitar_poligono_lotes.sql"
...
if [ -n "${DB_CONTAINER:-}" ]; then
  echo "Aplicando migracion de poligono de lotes en contenedor '${DB_CONTAINER}' (db=${DB_NAME}, user=${DB_USER})..."
...
if command -v psql >/dev/null 2>&1 && psql --version >/dev/null 2>&1; then
  echo "Aplicando migracion de poligono de lotes con cliente local psql (db=${DB_NAME}, user=${DB_USER})..."
