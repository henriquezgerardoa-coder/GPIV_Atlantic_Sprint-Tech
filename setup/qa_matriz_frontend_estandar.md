# Matriz QA Frontend Estandar

## Como usar

- Completa una fila por cada ejecucion de prueba.
- En `Evidencia` agrega ruta de captura, video o nota reproducible.
- En `Estado` usa: `OK`, `FAIL`, `BLOQ`.
- Si un caso falla, registra ticket o issue en `Observaciones`.
...
| ID | Pantalla | Caso de prueba | Resultado esperado | Evidencia | Estado | Observaciones |
|---|---|---|---|---|---|---|
...
## Criterios de aprobacion (smoke)

- Todos los casos `APP-001` a `APP-005` en `OK`.
- Al menos un caso de error por modulo en `OK` (`APP-007`, `APP-008`, `APP-009`).
- Flujos basicos de auth en `OK` (`REG-002`, `VER-001`).
